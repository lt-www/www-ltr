...
photos_page cf cs sm c nb =
    let rt = lt_root cf
        ...
        g_ = I.img_preview (rt,lt_base cf) cs c
        i_ = H.p [] [H.a [H.href f_] [H.img [H.src n_,H.alt n]]
                    ,H.br []
                    ,H.span [] [H.a [H.href f_] [H.cdata "high resolution file"]]
                    ,H.span [] c_
                    ,arrows cf nb]
        x_ = H.div [H.class' "content"] [g_,i_,I.img_preload 450 (rt,lt_base cf) cs c]
...
lt_write_photos_pages cf = do
  cs <- lt_img_data cf
  let sm = I.img_submenu (lt_base cf) cs
      f (c,n) = mapM (write_photo_page cf cs sm c) (I.sets n)
  mapM_ f cs
...
lt_photo_page cf (c,n) cs =
  let sm = I.img_submenu (lt_base cf) cs
      h = photos_page cf cs sm c (I.img_neighbours cs (c,n))
  in H.renderHTML5 h
...
lt_img_reductions cf = do
  ...
  mapM_ (I.img_mk_reduction (lt_dir cf)) nms
